...
scene.enter(ctx => {
    return ctx.reply(dedent`
        Hello, Welcome to the ⚡️KAPPA Airdrop!

        💸 You can get up to <b>250 000 000 KAPPA</b> for participating in our airdrop and <b>50 000 000 KAPPA</b> for each person you refer
...
scene.hears('Profile', async ctx => {
    return ctx.reply(dedent`
        ⚡️ <b>Balance:</b> ${numberWithSpaces(ctx.session.tokens)} KAPPA
        👥 <b>Referrals:</b> ${ctx.session.referrals.length}
        💼 <b>Wallet:</b> ${ctx.session.wallet ? `<code>${ctx.session.wallet}</code>` : 'Not registered'}

        Get <b>50 000 000 KAPPA</b> for each referral! <b>Your referral link:</b> https://t.me/slime_airdrop_bot?start=${ctx.from.id}
    `, { parse_mode: 'HTML', disable_web_page_preview: true })
})
